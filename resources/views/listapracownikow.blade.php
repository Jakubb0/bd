...
<body> 
    ...
    <tr>    
      ...
      <td>{{$data->nazwisko}}</td>
      <td>{{$data->telefon}}</td>
      <td>{{$data->data_dolaczenia}}</td>   
      <td>{{$data->ostatnio_aktywny}}</td>
      <td><form action="{{route('usun')}}"><button type="submit" class="btn btn-primary" value="{{$data->id}}" name="usun">Usuń</button></form></td>                   
    </tr>
	@endforeach
	</table>
